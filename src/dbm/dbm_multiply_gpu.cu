...
#define NUM_REGISTERS_SQUARED (NUM_REGISTERS * NUM_REGISTERS)
...
__device__ static void process_task(const int m, const int n, const int k,
  ...
  constexpr int K = (NUM_THREADS * NUM_REGISTERS_SQUARED) / imax(M, N);

  static_assert(M * N <= NUM_THREADS * NUM_REGISTERS_SQUARED,
                "Not enought threads to cover tile.");
  static_assert(K * imax(M, N) == NUM_THREADS * NUM_REGISTERS_SQUARED,
                "Wasting threads.");
  static_assert((M / NUM_REGISTERS) * NUM_REGISTERS == M,
                "M not divisible by NUM_REGISTERS");
  static_assert((N / NUM_REGISTERS) * NUM_REGISTERS == N,
                "N not divisible by NUM_REGISTERS");
  ...
  const int register_idx = (threadIdx.x * NUM_REGISTERS_SQUARED);
  const int y = register_idx / M;       // Can exceed N, guaranteed to reach K.
  const int x = register_idx - y * M;   // Fastest index, does not exceed M.
  const int xT = register_idx / N;      // Can exceed M, guaranteed to reach K.
  const int yT = register_idx - xT * N; // Fastest index, does not exceed N.
  ...
  for (int i_tile = 0; i_tile < m; i_tile += M) {
    for (int j_tile = 0; j_tile < n; j_tile += N) {
      double result[NUM_REGISTERS][NUM_REGISTERS] = {0.0};
      for (int l_tile = 0; l_tile < k; l_tile += K) {

        // Load tile_a from global into shared memory.
        if (x + NUM_REGISTERS < M && y + NUM_REGISTERS < K) {
#pragma unroll
          for (int ry = 0; ry < NUM_REGISTERS; ry++) {
#pragma unroll
            for (int rx = 0; rx < NUM_REGISTERS; rx++) {
              const int i = i_tile + x + rx;
              const int l = l_tile + y + ry;
              const int idx = l * m + i; // transa = "N"
              const bool load = (l < k && i < m);
              tile_a[(y + ry) * M + x + rx] = (load) ? block_a[idx] : 0.0;
            }
          }
        }
        ...
        if (yT + NUM_REGISTERS < N && xT + NUM_REGISTERS < K) {
#pragma unroll
          for (int rx = 0; rx < NUM_REGISTERS; rx++) {
#pragma unroll
            for (int ry = 0; ry < NUM_REGISTERS; ry++) {
              const int j = j_tile + yT + ry;
              const int l = l_tile + xT + rx;
              const int idx = l * n + j; // transb = "T"
              const bool load = (l < k && j < n);
              tile_b[(xT + rx) * N + yT + ry] = (load) ? block_b[idx] : 0.0;
            }
          }
        }
        ...
        __syncthreads();
        if (x + NUM_REGISTERS < M && y + NUM_REGISTERS < N) {
#pragma unroll
          for (int z = 0; z < K; z++) {
#pragma unroll
            for (int ry = 0; ry < NUM_REGISTERS; ry++) {
#pragma unroll
              for (int rx = 0; rx < NUM_REGISTERS; rx++) {
                result[rx][ry] +=
                    tile_a[z * M + x + rx] * tile_b[z * N + y + ry];
              }
            }
          }
        }
...
#pragma unroll
          for (int rx = 0; rx < NUM_REGISTERS; rx++) {
            const int i = i_tile + x + rx;
            const int j = j_tile + y + ry;
            if (i < m && j < n) {
              const int idx_c = j * m + i;
              // Needs atomics because other thread blocks might work on same C.
              atomicAddDouble(&block_c[idx_c], alpha * result[rx][ry]);
            }
          }
        }
      }
...
__global__ static void process_batch_kernel(const double alpha,
  ...
  __shared__ double shared_mem[2 * NUM_REGISTERS_SQUARED * NUM_THREADS];
  ...
  process_task<32, 32>(m, n, k, alpha, blk_a, blk_b, blk_c, shared_mem);
...
/*******************************************************************************
 * \brief Internal routine for uploading newly arrived packs onto the
 *device. \author Ole Schuett
 ******************************************************************************/
...
/*******************************************************************************
 * \brief Internal routine for executing the tasks in given batch on the
 *GPU. \author Ole Schuett
 ******************************************************************************/
