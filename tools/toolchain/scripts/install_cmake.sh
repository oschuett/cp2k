...
case "$cp2k_with_cmake" in
    ...
    *)
        ...
        pkg_install_dir="$cp2k_with_cmake"
        check_dir "${cp2k_with_cmake}/bin"
        ;;
esac
if [ "$cp2k_with_cmake" != "__DONTUSE__" ] ; then
    if [ "$cp2k_with_cmake" != "__SYSTEM__" ] ; then
